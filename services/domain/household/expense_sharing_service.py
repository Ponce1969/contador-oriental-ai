...
class ExpenseSharingService:
    ...
    def create_link(
        self, household_id: int, gasto_id: int, caller_familia_id: int
    ) -> SharedExpenseLink:
        ...
        self.audit_repo.append(
            household_id, caller_familia_id, gasto_id, action="created"
        )
        return link
    ...
    def delete_link(
        ...
        if existing.familia_id != caller_familia_id:
            raise UnauthorizedError(
                "Solo el usuario que compartió el gasto puede descompartirlo."
            )

        self.link_repo.delete_by_gasto_and_household(gasto_id, household_id)
        self.audit_repo.append(
            household_id, caller_familia_id, gasto_id, action="deleted"
        )

    def create_expense_and_link(
        self, household_id: int, expense_data: dict, caller_familia_id: int
    ) -> tuple[ExpenseTable, SharedExpenseLink]:
        if not self.member_repo.is_active_member(household_id, caller_familia_id):
            raise NotAMemberError("No sos miembro de este hogar.")
        ...
        try:
            ...
            self.audit_repo.append(
                household_id, caller_familia_id, expense.id, action="created"
            )
