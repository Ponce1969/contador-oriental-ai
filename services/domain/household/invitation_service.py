...
class InvitationService:
    ...
    def create_invitation(
        self, household_id: int, caller_familia_id: int
    ) -> HouseholdInvitation:
        ...
        if active_count >= 10:
            raise InvitationLimitError(
                "No se pueden tener más de 10 invitaciones activas simultáneamente."
            )
    ...
    def accept_invitation(self, token: str, joiner_familia_id: int) -> None:
        ...
        if invitation.status != "pending" or invitation.expires_at < datetime.now():
            raise InvalidInvitationError(
                "El link de invitación ha expirado o ya fue utilizado."
            )
        ...
        self.member_repo.add_member(
            invitation.household_id, joiner_familia_id, role="member"
        )
        self.invitation_repo.mark_accepted(token, joiner_familia_id)

    def revoke_invitation(
        self, household_id: int, token: str, caller_familia_id: int
    ) -> None:
        role = self.member_repo.get_member_role(household_id, caller_familia_id)
        if role != "admin":
        ...
        if invitation.status == "accepted":
            raise InvalidInvitationError(
                "No se puede revocar una invitación que ya fue aceptada."
            )
