...
class Expense(BaseModel):
    ...
    metodo_pago: PaymentMethod = Field(
        default=PaymentMethod.EFECTIVO, description="Método de pago utilizado"
    )

    # Recurrencia
    es_recurrente: bool = Field(
        default=False, description="Indica si es un gasto recurrente"
    )
    frecuencia: RecurrenceFrequency | None = Field(
        default=None, description="Frecuencia de recurrencia"
    )
