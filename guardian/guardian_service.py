...
DISCORD_WEBHOOK_URL: Final[str] = os.environ["DISCORD_WEBHOOK_URL"]
...
EXPECTED_CONTAINERS: Final[tuple[ContainerName, ...]] = (
    "auditor_familiar_app",
    "auditor_familiar_db",
    "auditor_familiar_ocr_api",
)
...
class Notifier(Protocol):
    ...
    async def send_message(
        self, title: str, description: str, color: DiscordColor
    ) -> None:
...
class ContainerMonitor:
    ...
    def get_status(self, container_name: str) -> ContainerCheckResult:
        ...
        try:
            ...
            is_healthy = state == ContainerState.RUNNING and health in (
    ...
    async def check_all(self) -> dict[str, ContainerStatus]:
        ...
        async with self._lock:
            ...
            self._last_status = {
                name: status.is_healthy for name, status in current_status.items()
            }
...
class GuardianService:
    ...
    async def _check_cuotas_ratio(self) -> None:
        ...
        try:
            ...
            # Ingresos promedio (ultimos 3 meses) en UYU
            ...
            for i in range(3):
                ...
                ingresos.append(
                    income_ctrl.get_total_by_month(anio, mes, currency="UYU").get(
                        "UYU", Decimal("0")
                    )
                )
            ...
            for mes_key, total_cuotas in proyeccion.items():
                ...
                if ratio > Decimal("0.3"):
                    logger.warning(
                        "ALERTA FINANCIERA: %s cuotas=%s "
                        "(%.0f%% del ingreso promedio %s)",
                        mes_key,
                        total_cuotas,
                        ratio * 100,
                        ingreso_promedio,
