...
def up(db):
    db.execute(
        "ALTER TABLE exchange_rates ADD COLUMN compra NUMERIC(10, 4) DEFAULT 0 NOT NULL"
    )
    db.execute(
        "ALTER TABLE exchange_rates ADD COLUMN venta NUMERIC(10, 4) DEFAULT 0 NOT NULL"
    )
...
def down(db):
    db.execute(
        "ALTER TABLE exchange_rates ADD COLUMN rate NUMERIC(10, 4) DEFAULT 0 NOT NULL"
    )
