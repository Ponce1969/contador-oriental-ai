...
def up(db):
    ...
    db.execute("CREATE INDEX idx_household_members_familia ON household_members(familia_id)")
    db.execute("CREATE INDEX idx_household_members_household ON household_members(household_id)")
    db.execute("CREATE INDEX idx_invitations_token ON household_invitations(token)")
    db.execute("CREATE INDEX idx_invitations_household_status ON household_invitations(household_id, status)")
    db.execute("CREATE INDEX idx_shared_links_household ON shared_expense_links(household_id)")
    db.execute("CREATE INDEX idx_shared_links_familia ON shared_expense_links(familia_id)")
    db.execute("CREATE INDEX idx_shared_links_gasto ON shared_expense_links(gasto_id)")
    db.execute("CREATE INDEX idx_settlements_household ON household_settlements(household_id)")
    db.execute("CREATE INDEX idx_settlements_payer ON household_settlements(payer_familia_id)")
    db.execute("CREATE INDEX idx_settlements_recipient ON household_settlements(recipient_familia_id)")
    db.execute("CREATE INDEX idx_settlements_fecha ON household_settlements(household_id, fecha)")
    db.execute("CREATE INDEX idx_audit_log_household ON household_audit_log(household_id)")
